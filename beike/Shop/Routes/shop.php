...
Route::prefix('/')
    ...
    ->group(function () {
        ...
        Route::post('files', [FileController::class, 'store'])->middleware('throttle:shop-uploads')->name('file.store');
        Route::get('forgotten', [ForgottenController::class, 'index'])->name('forgotten.index');
        Route::post('forgotten/send_code', [ForgottenController::class, 'sendVerifyCode'])->middleware('throttle:password-recovery')->name('forgotten.send_code');
        Route::post('forgotten/password', [ForgottenController::class, 'changePassword'])->middleware('throttle:password-recovery')->name('forgotten.password');

        Route::get('lang/{lang}', [LanguageController::class, 'index'])->name('lang.switch');

        Route::get('login', [LoginController::class, 'index'])->name('login.index');
        Route::post('login', [LoginController::class, 'store'])->middleware('throttle:shop-login')->name('login.store');
