              ...
              <td style="border: 1px solid #eee;padding:4px;font-size:13px;padding: 7px">
                ...
                <div style="display: flex; flex-wrap: wrap; align-items: center;">
                  @foreach ($rma['images'] as $image)
                    <a style="display: block; width: 60px; height: 60px; margin-right: 2px; margin-bottom: 2px; border: 1px solid #eee; border-radius: 4px; position: relative; display: flex; align-items: center; justify-content: center;" target="_blank" href="{{ image_origin($image) }}" data-bs-toggle="tooltip" title="{{ __('common.quick_view') }}"><img src="{{ image_resize($image, 200, 200) }}" style="width: 100%; max-height: 100%;"></a>
                  @endforeach
                </div>
              @else
        ...
        <dl style="font-size: 14px; color: rgb(51, 51, 51); line-height: 18px;">
          <dd style="margin: 0px 0px 6px; padding: 0px; font-size: 12px; line-height: 22px;">
            <p style="font-size: 14px; line-height: 26px; word-wrap: break-word; word-break: break-all; margin-top: 32px;">
              {{ __('mail.sincerely') }}
              <br>
              <strong>{{ system_setting('base.store_name', 'BeikeShop') }} {{ __('mail.team') }}</strong>
            </p>
          </dd>
        </dl>
